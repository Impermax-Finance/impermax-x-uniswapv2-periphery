...
contract('Router02 Staked', function (accounts) {
	...
	it('leverage', async () => {
		await expectRevert(
			leverage(router, stakedLPToken, borrower, '100', '8000', '90', '7000', '0x', '0x', ETH_IS_A),
			ETH_IS_A ? "ImpermaxRouter: INSUFFICIENT_A_AMOUNT" : "ImpermaxRouter: INSUFFICIENT_B_AMOUNT"
		);
		await expectRevert(
			leverage(router, stakedLPToken, borrower, '80', '10000', '70', '9000', '0x', '0x', ETH_IS_A),
			ETH_IS_A ? "ImpermaxRouter: INSUFFICIENT_B_AMOUNT" : "ImpermaxRouter: INSUFFICIENT_A_AMOUNT"
		);
		await expectRevert( 
			leverage(router, stakedLPToken, borrower, ETH_LEVERAGE_AMOUNT, UNI_LEVERAGE_AMOUNT, '0', '0', '0x', '0x', ETH_IS_A), 
		...
		await expectRevert( 
			leverage(router, stakedLPToken, borrower, ETH_LEVERAGE_AMOUNT_HIGH, 
		...
		const receipt = await leverage(router, stakedLPToken, borrower, ETH_LEVERAGE_AMOUNT, UNI_LEVERAGE_AMOUNT, '0', '0', permitBorrowETH, permitBorrowUNI, ETH_IS_A);
	...
	it('address calculation', async () => {
		...
		expect(await router.getBorrowable(stakedLPToken.address, '0')).to.eq(expectedBorrowableA);
		expect(await router.getBorrowable(stakedLPToken.address, '1')).to.eq(expectedBorrowableB);
		expect(await router.getCollateral(stakedLPToken.address)).to.eq(expectedCollateral);
		...
		const receipt = await router.getBorrowable.sendTransaction(stakedLPToken.address, '0');
